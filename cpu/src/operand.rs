...
impl Operand {
        ...
        let result = match self.size {
            3 => {
                format!("{:04X}: {:02X} {:02X} {:02X}   {} {}", pc,
                        self.opcode, b1, b2,
                        self.name,
                        self.addressing_type.to_string(pc, b1, b1 as u16 | (b2 as u16) << 8, labels))
            },
            2 => {
                format!("{:04X}: {:02X} {:02X}      {} {}", pc,
                        self.opcode, b1,
                        self.name,
                        self.addressing_type.to_string(pc, b1, 0, labels))
            },
            _ => {
                format!("{:04X}: {:02X}         {}", pc,
                        self.opcode,
    ...
    pub(crate) fn disassemble_bytes(pc: u16, operands: &[Operand], operand: u8, b1: u8, b2: u8,
            labels: &Labels) -> String
    {
        operands[operand as usize].disassemble(pc, b1, b2, labels)
    }
}
