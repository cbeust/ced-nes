...
pub trait Memory {
    ...
    fn disassemble(&mut self, operands: &[Operand], address: u16, labels: &Labels) -> DisassemblyLine {
        Disassemble::disassemble(&self.main_memory(), operands, address, labels)
    }
}
...
impl Memory for DefaultMemory {
    ...
    fn get_direct(&mut self, index: u16) -> u8 {
        self.buffer[index as usize]
    }
...
impl DefaultMemory {
    ...
    pub fn word_ind_y_mem(mem: &mut impl Memory, address: u16, ind_y: bool) -> u16 {
        let address_plus_one = if address == 0xff && ind_y { 0_u16 }
        else { address.wrapping_add(1) };
        (mem.get(address) as u16) | (mem.get(address_plus_one) as u16) << 8
    }
