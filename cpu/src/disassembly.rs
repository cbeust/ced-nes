...
impl RunDisassemblyLine {
    ...
    pub fn to_log(&self, labels: &Labels) -> Vec<String> {
        let mut result = Vec::new();
        if let Some(label) = labels.get(&self.disassembly_line.pc) {
            result.push(format!("@{label}:"));
        }
        let line = self.disassembly_line.to_asm();
        let flags = StatusFlags::new_with(self.p);
        result.push(format!("{:08} {:02X} {:02X} {:02X} {:04X} {flags} {:<10} | {}",
...
impl Disassemble {
    pub fn disassemble_range(memory: &[u8], operands: &[Operand], address: usize, stop: usize,
            labels: &Labels) -> Vec<DisassemblyLine>
    {
        let mut result: Vec<DisassemblyLine> = Vec::new();
        let mut current = address as u16;
        while current < stop as u16 {
            let dl = Disassemble::disassemble(memory, operands, current, labels);
    ...
    pub fn disassemble(memory: &[u8], operands: &[Operand], address: u16, labels: &Labels) -> DisassemblyLine {
        ...
        Disassemble::disassemble2(operands, address, op, byte1, byte2, labels)
    }
    
    pub fn disassemble2(operands: &[Operand], address: u16,
        op: &Operand, byte1: u8, byte2: u8, labels: &Labels)
            -> DisassemblyLine {
        let bytes: Vec<u8> =
            if op.size == 1 {
                Vec::new()
        ...
        let line = Operand::disassemble_bytes(address, operands, op.opcode, byte1, byte2, labels);
        ...
        DisassemblyLine {
            ...
            bytes,
            value: op.addressing_type.to_string(address, byte1, (byte2 as u16) << 8 | byte1 as u16, labels),
            line,
    ...
    pub fn disassemble_multiple(memory: &Vec<u8>, operands: &[Operand], mut index: u16,
        line_count: u16, labels: &Labels)
            -> Vec<DisassemblyLine> {
        let mut result: Vec<DisassemblyLine> = Vec::new();
        for _ in 0..line_count {
            let l = Disassemble::disassemble(memory, operands, index, labels);
