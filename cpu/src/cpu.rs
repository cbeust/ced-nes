...
pub static BREAKPOINT: Lazy<RwLock<bool>> = Lazy::new(|| RwLock::new(false));
...
pub struct Cpu<T: Memory> {
    ...
    pub p: StatusFlags,
    pub s: u8,
    /// How many cycles to wait before running the next instruction.
    pub wait_cycles: i8,
...
impl RunStatus {
    /// Return the number of cycles of the last instruction
...
impl <T: Memory> Cpu<T> {
        ...
        breakpoints: &HashSet<u16>) -> (bool, u8)
    {
        if self.wait_cycles == -1 || self.wait_cycles == 0 {
            ...
            if let Some((lsb, msb)) = self.pending_interrupt {
    ...
    fn address_value(&mut self, pc: u16, addressing_type: AddressingType)
        -> (u16, u8)
    {
        self.address_value_private(pc, addressing_type, false)
    }

    fn address_value_no_side_effect(&mut self, pc: u16, addressing_type: AddressingType)
        -> (u16, u8)
    {
        self.address_value_private(pc, addressing_type, true)
    }

    /// If direct is true, access the memory directly so we don't do side effects
    /// LDA needs to go through the getter so it can possibly trigger side effects but STA
    /// should not
    fn address_value_private(&mut self, pc: u16, addressing_type: AddressingType, direct: bool) -> (u16, u8) {
        let address = addressing_type.address(pc, self);
        // Actual address access here
        // if address == 0x2002 {
        //     println!("READING FROM 2002");
        // }
        let value = if direct {
            self.memory.get_direct(address)
        } else {
            self.memory.get(address)
        };
        // let value = self.memory.main_memory()[address as usize];
        (address, value)
    }
    ...
    fn run_indirect(&mut self, opcode: u8, pc: u16, address: u16,
        ...
        let result =
            if opcode == ind_y {
                let value = self.memory.get(pc.wrapping_add(1)) as u16;
                let word = DefaultMemory::word_ind_y_mem(&mut self.memory, value, opcode == ind_y);
                self.page_crossed(word, address)
            } else if opcode == abs_x || opcode == abs_y {
                let word = DefaultMemory::word_ind_y_mem(&mut self.memory, pc.wrapping_add(1),
                    opcode == ind_y);
                self.page_crossed(word, address)
            } else {
                0
        ...
        breakpoints: &HashSet<u16>)
    {
        ...
        /// we clear the carry.
        if config.system == System::Apple2 && pc == 0xc7b7 {
            self.p.set_c(false);
        }
        ...
        match opcode {
            ...
            ASL_ZP | ASL_ZP_X | ASL_ABS | ASL_ABS_X => {
                let (address, value) = self.address_value_no_side_effect(pc, addressing_type);
                resolved_address = Some(address);
                resolved_value = Some(value);
            ...
            INC_ZP | INC_ZP_X | INC_ABS | INC_ABS_X => {
                ...
                // INC is read-write-modify, so write back the value
                // Necessary for NES mappers to register a write
        ...
        // Adjust debug if there is a max_pc
        let is_below_max_pc = self.pc >= config.pc_max.unwrap_or(0);
    ...
    pub fn handle_interrupt(&mut self, brk: bool, vector_high: u16, vector_low: u16) {
        self.p.set_b(brk);

        let opcode = self.memory.get(self.pc);
        let operand = &self.operands[opcode as usize];
        let next_pc = self.pc; //  + operand.size as u16;
        // println!("handle_interrupt, current pc: {:04X} next pc: {:04X} opcode:{:02X}", self.pc,
        //     next_pc, opcode);
    ...
    pub fn nmi(&mut self) {
        self.pending_interrupt = Some((0xfffb, 0xfffa));
    }
